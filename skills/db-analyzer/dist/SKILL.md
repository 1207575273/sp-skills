...
命令统一是 `node scripts/run-query.bundle.mjs <参数>`,在本 skill 目录下执行。
...
```
node scripts/run-query.bundle.mjs --list-envs
```
...
```
node scripts/run-query.bundle.mjs --env <env> --sql "SELECT ..."
node scripts/run-query.bundle.mjs --env <env> --file query.sql        SQL 长时用文件
```
...
```
node scripts/run-query.bundle.mjs --env <env> --sql "EXPLAIN <原 SELECT>"
```
...
```
node scripts/run-query.bundle.mjs --env <env> --sql "SHOW VARIABLES LIKE 'slow_query%'"
node scripts/run-query.bundle.mjs --env <env> --sql "SELECT query, exec_count, avg_latency, rows_examined_avg FROM sys.statement_analysis ORDER BY avg_latency DESC LIMIT 20"
```
